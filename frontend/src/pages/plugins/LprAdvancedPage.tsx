...
export default function LprAdvancedPage() {
  ...
  const { data: statsData } = useQuery(
    ["lpradv-stats"],
    () => listEvents({ label: "lpr_advanced", source: "plugin", start: todayStart.toISOString(), limit: 500 }),
    { refetchInterval: 30000 },
  );
  const { data: blacklistStats } = useQuery(
    ["lpradv-bl-stats"],
    () => listEvents({ label: "blacklisted_plate", source: "plugin", start: todayStart.toISOString(), limit: 500 }),
    { refetchInterval: 30000 },
  );
  ...
  async function loadMore() {
    ...
    try {
      ...
      const data = await le({ label, source: "plugin", camera_id: cameraId !== "all" ? cameraId : undefined, cursor, limit: 50 });
  ...
  const filtered = search.trim()
    ? allEvents.filter((ev) => {
        ...
        const plate = (meta.plate as string) ?? (ev.sub_label ?? "");
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <div className="vms-card p-4 flex flex-col gap-1">
          ...
          <span className="text-sm font-semibold text-[var(--text-0)]">YOLO + PaddleOCR</span>
        </div>
      </div>
      ...
      <div className="vms-card">
        <div className="overflow-x-auto">
          <table className="vms-table">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={tab === "blacklisted" ? 6 : 5}>Cargando...</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={tab === "blacklisted" ? 6 : 5}>Sin registros</td></tr>
